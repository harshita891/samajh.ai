...
class Drawing:
    ...
    def draw_detections(self, frame, boxes, scores, class_ids, class_names, status_flags):
        for i, (box, score, class_id) in enumerate(zip(boxes, scores, class_ids)):
            x1, y1, x2, y2 = box
            color = (0, 255, 0) if status_flags[i] else (0, 0, 255)  # Green if in zone, Red if not
            cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
            cv2.putText(frame, f"{class_names[class_id]} {score:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            
            # Add alert if object is not in zone
            if not status_flags[i]:
                cv2.putText(frame, "ALERT: Object Outside Zone", (x1, y2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
        return frame
    ...
    def draw_tracking(self, frame, tracker_boxes):
        # Draw the tracking boxes in blue
